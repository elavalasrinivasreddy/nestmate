...
**Current focus:** Phase 2 (Auth) code-complete and build-verified (phone auth via OTP; both compile, dex, and unit-test clean — `./gradlew assembleDebug testDebugUnitTest lintDebug` all pass). Real-device phone testing and Firebase-console setup (enable provider, add SHA-1) are still yours to run. Phase 3 (Profile) is next.
